...
#define C_GPIO_PORT2_BUTON_LED_PIN GPIO_PIN2
...
void PORT1_IRQHandler(void)
{
    ...
    if(status & GPIO_PIN1)
    {
        GPIO_toggleOutputOnPin(GPIO_PORT_P2, C_GPIO_PORT2_BUTON_LED_PIN);
    }
    else if (status & GPIO_PIN4)
    {
        ...
        ccr1 += C_PWM_STEP;
        /* Make sure we did not cross a certain value */
        if (ccr1 > C_PWM_CCR0)
        {
            ccr1 = 0;
        }
...
{
    /* Setup RED LED P2.0 */
    GPIO_setAsOutputPin(GPIO_PORT_P2, C_GPIO_PORT2_BUTON_LED_PIN);
    GPIO_setOutputLowOnPin(GPIO_PORT_P2, C_GPIO_PORT2_BUTON_LED_PIN);
    ...
    /* Setup the Button P1.4 with interrupt. */
